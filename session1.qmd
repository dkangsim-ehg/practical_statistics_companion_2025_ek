...
print(head(diet_data,8)) # show first 8 rows.
```
...
# summary_result <- summarise(group_by(diet_data, EXERCISE), mean_kcal = mean(KCAL3, na.rm = TRUE))

# With pipes (easy to read):
diet_data %>%
  group_by(EXERCISE) %>%
  summarise(mean_kcal = round(mean(KCAL3, na.rm = TRUE), 0), .groups = "drop")
```

**Clinical Insight**: The pipe `%>%` makes code read like instructions: "Take the data, THEN group by exercise, THEN calculate mean calories."
...
```{webr-r}
# First, examine the structure of our dataset
str(diet_data)
...
variable_classification <- tibble(
  Variable = c("ID", "SEX", "FDWT3", "KCAL3", "PROT3GM", "FAT3GM", "CHO3GM", "EXERCISE"),
...
# Display using gtsummary-style formatting
...
**Clinical Answer**: Nominal variables (ID, SEX, EXERCISE) are used for grouping and counting. Ratio variables (all nutritional measures) allow mathematical operations like means and correlations.
...
```{webr-r}
# Examine the distribution of caloric intake
...
```{webr-r}
# Compare protein intake across exercise groups using boxplots
...
```{webr-r}
# Final comprehensive table showing all variables
